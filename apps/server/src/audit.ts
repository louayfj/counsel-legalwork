...
function resolveLegalworkDataDir(): string {
  const override = process.env.LEGALWORK_DATA_DIR?.trim();
  if (override) return expandHome(override);
  return join(homedir(), ".legalwork", "legalwork-server");
}

export function auditLogPath(workspaceId: string): string {
  return join(resolveLegalworkDataDir(), "audit", `${workspaceId}.jsonl`);
}

export function legacyAuditLogPath(workspaceRoot: string): string {
  return join(workspaceRoot, ".opencode", "legalwork", "audit.jsonl");
}
