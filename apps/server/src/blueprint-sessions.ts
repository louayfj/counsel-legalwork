...
export function normalizeBlueprintSessionTemplates(legalwork: Record<string, unknown> | null | undefined): BlueprintSessionTemplate[] {
  const blueprint = readRecord(legalwork?.blueprint);
...
export function readMaterializedBlueprintSessions(legalwork: Record<string, unknown> | null | undefined): MaterializedBlueprintSession[] {
  const blueprint = readRecord(legalwork?.blueprint);
...
export function sanitizeLegalworkTemplateConfig(legalwork: Record<string, unknown> | null | undefined): Record<string, unknown> {
  const next = cloneRecord(legalwork ?? {});
...
export function applyMaterializedBlueprintSessions(
  legalwork: Record<string, unknown> | null | undefined,
  items: MaterializedBlueprintSession[],
  hydratedAt: number,
): Record<string, unknown> {
  const next = sanitizeLegalworkTemplateConfig(legalwork);
  const blueprint = readRecord(next.blueprint) ?? {};
  const materialized = readRecord(blueprint.materialized) ?? {};
