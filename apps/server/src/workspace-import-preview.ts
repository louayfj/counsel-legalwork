...
export type WorkspaceImportChangeKind = "opencode" | "legalwork" | "skill" | "command" | "file";
...
type WorkspaceImportSection = "opencode" | "legalwork" | "skills" | "commands" | "files";

const CORE_SKILL_NAMES = new Set<string>();
const CORE_COMMAND_NAMES = new Set<string>();
const CORE_PORTABLE_FILE_PATHS = new Set<string>();

for (const file of CORE_OPENCODE_FILES) {
  CORE_PORTABLE_FILE_PATHS.add(file.path);
  const skillMatch = /^\.opencode\/skills\/([^/]+)\/SKILL\.md$/.exec(file.path);
  if (skillMatch?.[1]) CORE_SKILL_NAMES.add(skillMatch[1]);
  const commandMatch = /^\.opencode\/commands\/([^/]+)\.md$/.exec(file.path);
  if (commandMatch?.[1]) CORE_COMMAND_NAMES.add(commandMatch[1]);
}
...
function normalizeModes(value: unknown): Record<string, WorkspaceImportMode> {
  ...
  return {
    ...
    legalwork: readMode(record.legalwork),
...
export function normalizeWorkspaceImportPayload(
  ...
  return {
    ...
    sections: {
      ...
      legalwork: payload.legalwork !== undefined,
    ...
    ...(payload.opencode !== undefined
      ...
      : {}),
    ...(payload.legalwork !== undefined
      ? { legalwork: sanitizeLegalworkTemplateConfig(readRecord(payload.legalwork)) }
      : {}),
...
export async function buildWorkspaceImportPreview(
  ...
  if (input.legalwork !== undefined) {
    ...
    changes.push({
      kind: "legalwork",
      action: actionForTarget(existsBefore, !sameJson(before, after), input.modes.legalwork),
      label: "LegalWork config",
      path: rel(workspaceRoot, path),
      absolutePath: path,
