...
<html lang="en">
  <head>
    ...
    <title>LegalWork Toy UI</title>
    <link rel="icon" type="image/svg+xml" href="/ui/assets/legalwork-mark.svg" />
  ...
  <body>
    <div class="wrap">
      <div class="top">
        <div class="title">
          <h1>LegalWork Toy UI</h1>
          <div class="sub">Local-first host contract harness (served by legalwork-server)</div>
        </div>
        <div class="row">
        ...
        <div class="card">
          ...
          <div class="body">
            ...
            <div class="panel hidden" data-panel="config">
              ...
              <div class="small">Uploads go to <span class="mono">.opencode/legalwork/inbox/</span> inside the workspace.</div>

              <div class="hr"></div>

              <div class="row">
                <button class="btn" id="btn-artifacts">List artifacts</button>
                <span class="small">Downloads read from <span class="mono">.opencode/legalwork/outbox/</span>.</span>
              </div>
...
export const TOY_UI_FAVICON_SVG = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 96 96" fill="none"><rect width="96" height="96" rx="20" fill="#0E0A07"/><rect x="14" y="14" width="32" height="32" fill="#2352DE"/><rect x="50" y="14" width="32" height="32" fill="#FEFEFE"/><rect x="50" y="50" width="32" height="32" fill="#8669B9"/></svg>`;
...
const STORAGE_TOKEN = "legalwork.toy.token";
const STORAGE_SESSION_PREFIX = "legalwork.toy.session.";
...
async function ensureSession(workspaceId) {
  ...
  const created = await apiFetch("/w/" + encodeURIComponent(workspaceId) + "/opencode/session", {
    method: "POST",
    body: JSON.stringify({ title: "LegalWork Toy UI" }),
  });
...
async function main() {
  ...
  qs("#btn-delete-workspace").onclick = async () => {
    if (!confirm("Delete this workspace from the host's LegalWork server config?")) return;
    try {
