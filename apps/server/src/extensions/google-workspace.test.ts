...
function createTestConfig(): ServerConfig {
  const tempDir = join(
    tmpdir(),
    `legalwork-google-workspace-test-${Date.now()}-${Math.random().toString(36).slice(2)}`,
  );
  return {
    host: "127.0.0.1",
    port: 8787,
...
const previousEnv = {
  devMode: process.env.LEGALWORK_DEV_MODE,
  plaintextVault: process.env.LEGALWORK_GOOGLE_WORKSPACE_ALLOW_PLAINTEXT_VAULT,
  clientId: process.env.LEGALWORK_GOOGLE_WORKSPACE_OAUTH_CLIENT_ID,
  legacyClientId: process.env.GOOGLE_WORKSPACE_OAUTH_CLIENT_ID,
  clientSecret: process.env.GOOGLE_WORKSPACE_OAUTH_CLIENT_SECRET,
  legacyClientSecret: process.env.LEGALWORK_GOOGLE_WORKSPACE_OAUTH_CLIENT_SECRET,
  brokerUrl: process.env.LEGALWORK_GOOGLE_WORKSPACE_TOKEN_BROKER_URL,
  legacyBrokerUrl: process.env.GOOGLE_WORKSPACE_TOKEN_BROKER_URL,
};
...
afterEach(() => {
  restoreEnv("LEGALWORK_DEV_MODE", previousEnv.devMode);
  restoreEnv("LEGALWORK_GOOGLE_WORKSPACE_ALLOW_PLAINTEXT_VAULT", previousEnv.plaintextVault);
  restoreEnv("LEGALWORK_GOOGLE_WORKSPACE_OAUTH_CLIENT_ID", previousEnv.clientId);
  restoreEnv("GOOGLE_WORKSPACE_OAUTH_CLIENT_ID", previousEnv.legacyClientId);
  restoreEnv("GOOGLE_WORKSPACE_OAUTH_CLIENT_SECRET", previousEnv.clientSecret);
  restoreEnv("LEGALWORK_GOOGLE_WORKSPACE_OAUTH_CLIENT_SECRET", previousEnv.legacyClientSecret);
  restoreEnv("LEGALWORK_GOOGLE_WORKSPACE_TOKEN_BROKER_URL", previousEnv.brokerUrl);
  restoreEnv("GOOGLE_WORKSPACE_TOKEN_BROKER_URL", previousEnv.legacyBrokerUrl);
  globalThis.fetch = previousFetch;
});

describe("Google Workspace extension", () => {
  test("reports OAuth client id and secret as missing when unconfigured", async () => {
    process.env.LEGALWORK_GOOGLE_WORKSPACE_OAUTH_CLIENT_ID = "";
    process.env.GOOGLE_WORKSPACE_OAUTH_CLIENT_ID = "";
    process.env.GOOGLE_WORKSPACE_OAUTH_CLIENT_SECRET = "";
    process.env.LEGALWORK_GOOGLE_WORKSPACE_OAUTH_CLIENT_SECRET = "";
    process.env.LEGALWORK_GOOGLE_WORKSPACE_TOKEN_BROKER_URL = "";
    process.env.GOOGLE_WORKSPACE_TOKEN_BROKER_URL = "";
    const status = await googleWorkspaceStatus(createTestConfig());
    expect(status.configured).toBe(false);
    expect(status.missing).toEqual([
      "LEGALWORK_GOOGLE_WORKSPACE_OAUTH_CLIENT_ID",
      "GOOGLE_WORKSPACE_OAUTH_CLIENT_SECRET",
    ]);
  });

  test("treats configured env OAuth clients as custom clients", async () => {
    process.env.LEGALWORK_GOOGLE_WORKSPACE_OAUTH_CLIENT_ID = "client-id";
    process.env.GOOGLE_WORKSPACE_OAUTH_CLIENT_SECRET = "secret";
    process.env.LEGALWORK_GOOGLE_WORKSPACE_TOKEN_BROKER_URL = "";
    const status = await googleWorkspaceStatus(createTestConfig());
    expect(status.configured).toBe(true);
    expect(status.missing).toEqual([]);
    expect(status.customClient).toBe(true);
  });

  test("reads multi-account vaults and exposes active account", async () => {
    process.env.LEGALWORK_DEV_MODE = "1";
    process.env.LEGALWORK_GOOGLE_WORKSPACE_ALLOW_PLAINTEXT_VAULT = "1";
    process.env.GOOGLE_WORKSPACE_OAUTH_CLIENT_SECRET = "secret";
  ...
  test("connect start requires OAuth config before optional features", async () => {
    process.env.LEGALWORK_GOOGLE_WORKSPACE_OAUTH_CLIENT_ID = "";
    process.env.GOOGLE_WORKSPACE_OAUTH_CLIENT_ID = "";
    process.env.GOOGLE_WORKSPACE_OAUTH_CLIENT_SECRET = "secret";
    process.env.LEGALWORK_GOOGLE_WORKSPACE_TOKEN_BROKER_URL = "";
    ...
    expect(flows.start({ features: ["driveFull"] })).rejects.toThrow(
      new ApiError(400, "google_oauth_not_configured", "Missing Google OAuth configuration: LEGALWORK_GOOGLE_WORKSPACE_OAUTH_CLIENT_ID"),
    );
