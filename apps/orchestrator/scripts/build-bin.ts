...
const serverBin = resolve(root, "..", "server", "dist", "bin", "legalwork-server");
...
await copyFile(serverBin, resolve(targetDir, "legalwork-server"));
...
const versions = {
  "legalwork-server": {
    ...
    sha256: await sha256(resolve(targetDir, "legalwork-server")),
  },
  "opencode-router": {
