...
if (!orchestratorVersion) {
  throw new Error(
    "legalwork-orchestrator version missing in apps/orchestrator/package.json",
  );
}
...
if (!serverVersion) {
  throw new Error(
    "legalwork-server version missing in apps/server/package.json",
  );
}
...
run("pnpm", ["--filter", "legalwork-server", "build:bin:all"], repoRoot);
...
const entries = {
  "legalwork-server": { version: serverVersion, targets: {} },
  "opencode-router": { version: routerVersion, targets: {} },
};

for (const target of targets) {
  const ext = target.id.startsWith("windows") ? ".exe" : "";
  const serverSrc = join(serverDir, `legalwork-server-${target.bun}${ext}`);
  if (!existsSync(serverSrc)) {
    throw new Error(`Missing legalwork-server binary at ${serverSrc}`);
  }
  const serverDest = join(outdir, `legalwork-server-${target.id}${ext}`);
  ...
  entries["legalwork-server"].targets[target.id] = {
...
writeFileSync(
  join(outdir, "legalwork-orchestrator-sidecars.json"),
