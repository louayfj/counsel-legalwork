...
async function runCli(args) {
  ...
  if (code !== 0) {
    throw new Error(stderr.trim() || `legalwork CLI failed with code ${code}`);
  }
...
const root = await mkdtemp(join(tmpdir(), "legalwork-file-session-"));
...
try {
  await waitFor(`${legalworkUrl}/health`);

  const workspaces = await fetchJson(`${legalworkUrl}/workspaces`, {
  ...
  assert.equal(diskContent, "updated by legalwork cli\n");
  ...
  console.log(JSON.stringify({ ok: true, legalworkUrl, workspaceId, sessionId }, null, 2));
} catch (error) {
  console.error(
    JSON.stringify(
      {
