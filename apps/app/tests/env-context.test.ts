...
import {
  buildLegalworkEnvSystemContext,
  clearLegalworkEnvSystemContextCache,
} from "../src/react-app/domains/session/sync/env-context";

function client(keys: string[], calls: { count: number }): LegalworkServerClient {
  return {
...
describe("buildLegalworkEnvSystemContext", () => {
  test("lists configured key names without inventing secret values", async () => {
    clearLegalworkEnvSystemContextCache();
    const calls = { count: 0 };
    const context = await buildLegalworkEnvSystemContext(
  ...
  test("caches key context per session", async () => {
    ...
    await buildLegalworkEnvSystemContext(server, {
      cacheKey: "session-a",
      readPendingChanges: () => false,
    });
    await buildLegalworkEnvSystemContext(server, {
  ...
  test("does not truncate long key lists", async () => {
    ...
    const context = await buildLegalworkEnvSystemContext(client(keys, calls), {
      cacheKey: "session-a",
      readPendingChanges: () => false,
    });
  ...
  test("skips context while environment changes are pending", async () => {
    clearLegalworkEnvSystemContextCache();
    const calls = { count: 0 };
    const context = await buildLegalworkEnvSystemContext(client(["ANTHROPIC_API_KEY"], calls), {
      cacheKey: "session-a",
