...
function readStoredList(): string[] {
  ...
  try {
    const raw = window.localStorage.getItem("legalwork.server.list");
...
function readStoredActive(): string {
  if (typeof window === "undefined") return "";
  try {
    const stored = window.localStorage.getItem("legalwork.server.active");
...
function readLegalworkToken(): string {
  if (typeof window === "undefined") return "";
  try {
    return (window.localStorage.getItem("legalwork.server.token") ?? "").trim();
  } catch {
    return "";
  }
}
...
async function checkHealth(url: string): Promise<boolean> {
  if (!url) return false;
  const token = readLegalworkToken();
  const headers =
...
export function ServerProvider({ children, defaultUrl }: ServerProviderProps) {
    ...
    const forceProxy =
      ...
      (import.meta.env.PROD ||
        (typeof import.meta.env?.VITE_LEGALWORK_URL === "string" &&
          import.meta.env.VITE_LEGALWORK_URL.trim().length > 0));
  ...
  useEffect(() => {
    ...
    if (typeof window === "undefined") return;
    try {
      window.localStorage.setItem("legalwork.server.list", JSON.stringify(list));
      window.localStorage.setItem("legalwork.server.active", active);
    } catch {
      // ignore
    }
  }, [active, list]);
