...
import { relaunchDesktopApp, resetLegalworkState } from "../../app/lib/desktop";
import {
...
export type ResetState = {
  ...
  resetModalMode: ResetLegalworkMode;
...
export type SystemStateControls = {
  ...
  reset: ResetState;
  openResetModal: (mode: ResetLegalworkMode) => void;
...
function clearLegalworkLocalStorage(mode: ResetLegalworkMode) {
  ...
  try {
    ...
    for (const key of keys) {
      if (/legalwork/.test(key)) window.localStorage.removeItem(key);
    }
    window.localStorage.removeItem("legalwork_mode_pref");
  } catch {
    // ignore
  }
}
...
export function useSystemState(
  ...
  const [resetModalMode, setResetModalMode] =
    useState<ResetLegalworkMode>("onboarding");
  ...
  const reloadWorkspaceEngine = useCallback(async () => {
    ...
    if (options.canReloadWorkspaceEngine?.() === false) {
      // No reloadable workspace engine right now (e.g. a global skills/MCP/plugin change
      // made with no active workspace). The change is already persisted to the shared
      // global config and loads automatically when a workspace engine starts — so dismiss
      // the pending-reload state quietly instead of surfacing a hard error.
      clearReloadRequired();
      return;
    }
    setReloadBusy(true);
  ...
  const confirmReset = useCallback(async () => {
    ...
    try {
      if (isDesktopRuntime()) {
        await resetLegalworkState(resetModalMode);
      }
      clearLegalworkLocalStorage(resetModalMode);
      if (isDesktopRuntime()) {
