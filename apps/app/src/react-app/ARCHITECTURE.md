...
2. Leaf modules (`runtime-env`, `desktop-types`, `den-types`, `extensions`)
   ...
   (`opencode`, `legalwork-server`, `den`) import leaves — never the `utils/`
...
- The legacy `legalwork.react.activeWorkspace` and
  `legalwork.react.sessionByWorkspace` values are only restore/fallback memory.
...
- E2E: `pnpm test:e2e` from the repo root drives the real app checks.
