  ...
  return (
    <div className="-mr-1 mt-4 min-h-0 flex-1 space-y-4 overflow-y-auto pr-1">
      ...
      {props.renderedCount === 0 ? (
        <div className="rounded-2xl border border-dls-border bg-dls-hover px-4 py-6 text-sm text-dls-secondary">
      ...
      <div className="flex items-start gap-3">
        <ProviderIcon
          ...
          className={[
            ...
            active ? "text-dls-text" : "text-dls-secondary group-hover:text-dls-text",
          ].join(" ")}
        />
        <div className="flex-1 min-w-0">
          <div className={["text-[13px] flex items-center justify-between gap-2", active ? "font-medium text-dls-text" : "text-current"].join(" ")}>
            <span className="truncate">{opt.title}</span>
            <span className="flex shrink-0 items-center gap-1.5">
              {opt.source === "cloud" ? (
                <span className="rounded-md bg-[rgba(var(--dls-accent-rgb),0.06)] px-1.5 py-0.5 text-[10px] font-medium text-dls-accent">
                  Cloud
                </span>
              ) : null}
              {active ? (
                <span className="rounded-md bg-emerald-3 px-1.5 py-0.5 text-[10px] font-medium text-emerald-11">
                  Current
                </span>
              ) : null}
            </span>
          </div>
          <div className={["mt-0.5 flex items-center gap-3 text-[11px]", active ? "text-dls-secondary" : "text-dls-secondary group-hover:text-dls-text"].join(" ")}>
          ...
          {opt.footer ? (
            <div className={["text-[11px] mt-1", active ? "text-dls-secondary" : "text-dls-secondary group-hover:text-dls-text"].join(" ")}>
              {opt.footer}
            </div>
          ) : null}
...
function ModelBehaviorOptions(props: {
  ...
  return (
    <div role="presentation" className="mt-3 flex items-center gap-2" onClick={(event) => event.stopPropagation()} onKeyDown={(event) => event.stopPropagation()}>
      <span className="text-[11px] font-medium text-dls-secondary mr-1">{props.opt.behaviorTitle}:</span>
        ...
        {(props.opt.behaviorOptions ?? []).map((option) => (
          <button
            ...
            className={[
              ...
              props.opt.behaviorValue === option.value ? "text-dls-text font-semibold" : "text-dls-secondary hover:text-dls-text",
            ].join(" ")}
            onClick={(event) => {
              event.preventDefault();
...
function ProviderLinkRow(props: {
  ...
  return (
    <div
      ...
      className={[
        "group w-full text-left rounded-xl px-3 py-2.5 transition-colors cursor-pointer",
        isKeyboardActive ? "bg-dls-hover text-dls-text" : "text-dls-secondary hover:bg-dls-hover hover:text-dls-text",
      ].join(" ")}
    ...
    >
      <div className="flex items-start gap-3">
        <ProviderIcon
          ...
          className={[
            "mt-[1px] shrink-0 transition-colors",
            isKeyboardActive ? "text-dls-text" : "text-dls-secondary group-hover:text-dls-text",
          ].join(" ")}
        />
        <div className="flex-1 min-w-0">
          ...
          <div className={["mt-0.5 flex items-center gap-3 text-[11px]", isKeyboardActive ? "text-dls-secondary" : "text-dls-secondary group-hover:text-dls-text"].join(" ")}>
