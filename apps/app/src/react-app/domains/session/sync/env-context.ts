...
import { readLegalworkEnvPendingChanges } from "../../../../app/lib/legalwork-env-runtime";

const DEFAULT_CACHE_KEY = "__legalwork_env_default__";
const MAX_CONTEXT_CACHE_ENTRIES = 100;

const envSystemContextCache = new Map<string, string | undefined>();

export function clearLegalworkEnvSystemContextCache(): void {
...
export async function buildLegalworkEnvSystemContext(
  ...
  const readPendingChanges = options.readPendingChanges ??
    (() => readLegalworkEnvPendingChanges(options.runtimeKey));
