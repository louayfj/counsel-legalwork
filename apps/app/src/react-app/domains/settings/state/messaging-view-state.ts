...
import {
  ...
  type LegalworkOpenCodeRouterHealthSnapshot,
  type LegalworkOpenCodeRouterIdentityItem,
  type LegalworkOpenCodeRouterSendResult,
  type LegalworkServerClient,
  type LegalworkServerStatus,
  type LegalworkWorkspaceFileContent,
...
function isOpenCodeRouterSnapshot(
  value: unknown,
): value is LegalworkOpenCodeRouterHealthSnapshot {
...
function isOpenCodeRouterIdentities(
  value: unknown,
): value is { ok: boolean; items: LegalworkOpenCodeRouterIdentityItem[] } {
  if (!value || typeof value !== "object") return false;
  const record = value as Record<string, unknown>;
...
function readMessagingEnabledFromLegalworkConfig(value: unknown): boolean {
...
type UseMessagingViewPropsOptions = {
  ...
  reconnectLegalworkServer: () => Promise<boolean>;
...
export function useMessagingViewProps(
  ...
  const [health, setHealth] = useState<LegalworkOpenCodeRouterHealthSnapshot | null>(null);
  const [healthError, setHealthError] = useState<string | null>(null);
  const [telegramIdentities, setTelegramIdentities] = useState<LegalworkOpenCodeRouterIdentityItem[]>([]);
  ...
  const [sendResult, setSendResult] =
    useState<LegalworkOpenCodeRouterSendResult | null>(null);
  ...
  const scopedLegalworkBaseUrl = useMemo(() => {
    const baseUrl = options.legalworkServerUrl.trim();
    if (!baseUrl) return "";
    return buildLegalworkWorkspaceBaseUrl(baseUrl, workspaceId) ?? baseUrl;
  }, [options.legalworkServerUrl, workspaceId]);
  const serverReady =
    options.legalworkServerStatus === "connected" &&
    Boolean(options.legalworkServerClient);
  ...
  const loadAgentFile = useCallback(async () => {
    ...
    const client = options.legalworkServerClient;
    if (!client) return;
    ...
    try {
      ...
      const result = (await client.readWorkspaceFile(
        id,
        OPENCODE_ROUTER_AGENT_FILE_PATH,
      )) as LegalworkWorkspaceFileContent;
      ...
      if (error instanceof LegalworkServerError && error.status === 404) {
        setAgentExists(false);
        setAgentContent("");
        setAgentDraft("");
        setAgentBaseUpdatedAt(null);
  ...
  }, [agentLoading, options.legalworkServerClient, resetAgentState, serverReady, workspaceId]);
  ...
  }, [agentSaving, options.legalworkServerClient, serverReady, workspaceId]);
  ...
  const saveAgentFile = useCallback(async () => {
    ...
    try {
      ...
      if (error instanceof LegalworkServerError && error.status === 409) {
  ...
  }, [agentBaseUpdatedAt, agentDraft, agentSaving, options.legalworkServerClient, serverReady, workspaceId]);
  ...
  const sendTestMessage = useCallback(async () => {
    ...
    try {
      ...
      })) as LegalworkOpenCodeRouterSendResult;
  ...
  const refreshAll = useCallback(async (nextOptions?: { force?: boolean }) => {
    ...
    try {
      ...
      const isModuleEnabled = readMessagingEnabledFromLegalworkConfig(config?.legalwork);
  ...
  const repairAndReconnect = useCallback(async () => {
    if (options.legalworkReconnectBusy) return;
    setReconnectStatus(null);
    setReconnectError(null);

    const ok = await options.reconnectLegalworkServer();
    if (!ok) {
  ...
  }, [messagingSaving, options.legalworkServerClient, refreshAll, serverReady, workspaceId]);
  ...
  }, [options.legalworkServerClient, refreshAll, serverReady, telegramSaving, workspaceId]);
  ...
  }, [resetAgentState, scopedLegalworkBaseUrl, workspaceId]);
  ...
  }, [scopedLegalworkBaseUrl, serverReady, workspaceId]);

  return {
    busy: options.busy,
    showHeader: false,
    legalworkServerStatus: options.legalworkServerStatus,
    legalworkServerUrl: options.legalworkServerUrl,
    scopedLegalworkBaseUrl,
    workspaceId,
    selectedWorkspaceRoot: options.selectedWorkspaceRoot,
    refreshing,
    legalworkReconnectBusy: options.legalworkReconnectBusy,
    reconnectStatus,
    reconnectError,
    health,
    healthError,
