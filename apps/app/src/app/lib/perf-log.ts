...
type PerfRoot = typeof globalThis & {
  __legalworkPerfSeq?: number;
  __legalworkPerfLogs?: PerfLogRecord[];
  __legalworkPerfConsoleAt?: Record<string, number>;
  __legalworkPerfConsoleSuppressed?: Record<string, number>;
};
...
export const recordPerfLog = (
  ...
  const id = (root.__legalworkPerfSeq ?? 0) + 1;
  root.__legalworkPerfSeq = id;
  ...
  try {
    ...
    const now = Date.now();
    const lastByKey = root.__legalworkPerfConsoleAt ?? (root.__legalworkPerfConsoleAt = {});
    const suppressedByKey =
      root.__legalworkPerfConsoleSuppressed ?? (root.__legalworkPerfConsoleSuppressed = {});
...
export const clearPerfLogs = () => {
  const root = globalThis as PerfRoot;
  root.__legalworkPerfLogs = [];
  root.__legalworkPerfSeq = 0;
};
