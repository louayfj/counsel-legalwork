import { normalizeLegalworkServerUrl } from "./legalwork-server";
...
function isSupportedDeepLinkProtocol(protocol: string): boolean {
  ...
  return normalized === "legalwork:" || normalized === "legalwork-dev:" || normalized === "https:" || normalized === "http:";
}
...
export function parseRemoteConnectDeepLink(rawUrl: string): RemoteWorkspaceDefaults | null {
  ...
  const hostUrlRaw = url.searchParams.get("legalworkHostUrl") ?? url.searchParams.get("legalworkUrl") ?? "";
  const tokenRaw = url.searchParams.get("legalworkToken") ?? url.searchParams.get("accessToken") ?? "";
  const normalizedHostUrl = normalizeLegalworkServerUrl(hostUrlRaw);
  ...
  return {
    legalworkHostUrl: normalizedHostUrl,
    legalworkToken: token,
    directory: null,
...
function normalizeDebugDeepLinkInput(rawValue: string): string {
  ...
  const directMatch = trimmed.match(/(?:legalwork-dev|legalwork|https?):\/\/[^\s"'<>]+/i);
