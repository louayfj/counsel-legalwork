 ...
 *
 * Consumers read via `window.__legalwork` from a devtools console or from
...
declare global {
  ...
  var __legalwork: InspectorAPI | undefined;
}
...
function installIfNeeded() {
  ...
  Object.defineProperty(window, "__legalwork", {
    value: api,
    configurable: true,
    writable: false,
  });
...
export function recordInspectorEvent(name: string, data?: unknown) {
  installIfNeeded();
  window.__legalwork?.record(name, data);
}
