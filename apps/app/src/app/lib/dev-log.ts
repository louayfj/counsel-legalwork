...
type DevRoot = typeof globalThis & {
  __legalworkDevLogSeq?: number;
  __legalworkDevLogs?: DevLogRecord[];
};
...
export const recordDevLog = (
  ...
  const id = (root.__legalworkDevLogSeq ?? 0) + 1;
  root.__legalworkDevLogSeq = id;
...
export const clearDevLogs = () => {
  const root = globalThis as DevRoot;
  root.__legalworkDevLogs = [];
  root.__legalworkDevLogSeq = 0;
};
