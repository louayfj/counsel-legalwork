...
export const MIGRATION_KEY_PATTERNS: Array<RegExp> = [
  /^legalwork\.react\.activeWorkspace$/,
  /^legalwork\.react\.sessionByWorkspace$/,
  /^legalwork\.server\.list$/,
  /^legalwork\.server\.active$/,
  /^legalwork\.server\.urlOverride$/,
  /^legalwork\.server\.token$/,
];
...
function electronMigrationBridge(): ElectronMigrationBridge | null {
  ...
  const bridge = (window as unknown as {
    __LEGALWORK_ELECTRON__?: { migration?: ElectronMigrationBridge };
...
export const MIGRATION_DEFER_KEY = "legalwork.migration.deferredUntil";
