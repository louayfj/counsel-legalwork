const PENDING_CHANGES_KEY = "legalwork.settings.environment.pendingChanges";
...
export function buildLegalworkEnvRuntimeKey(input: {
  baseUrl?: string | null;
  pid?: number | null;
  port?: number | null;
}): string | undefined {
  ...
  return `${baseUrl || "legalwork"}::${runtime || "runtime"}`;
}

export function readLegalworkEnvPendingChanges(runtimeKey?: string | null): boolean {
...
export function writeLegalworkEnvPendingChanges(value: boolean, runtimeKey?: string | null): void {
  const localStorage = getStorage("localStorage");
  const sessionStorage = getStorage("sessionStorage");
  try {
    if (value) {
