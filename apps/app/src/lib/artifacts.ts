...
// OOXML wordprocessing formats the in-app docx editor can render; other "document"
// types (.doc, .odt, .rtf, .pages) fall back to "external" (open in Word).
const WORD_PREVIEW_EXTENSIONS = ["docx", "docm", "dotx", "dotm"];

function artifactTypeToPreview(type: ArtifactType, extension?: string): OpenTargetPreview {
  if (type === "markdown") return "markdown";
  if (type === "sheet") return "sheet";
  if (type === "slides") return "slides";
  if (type === "document") return extension && WORD_PREVIEW_EXTENSIONS.includes(extension) ? "word" : "external";
...
function openTargetFromArtifactPath(
  ...
  return verified ?? {
    ...
    value: normalized,
    name,
    preview: artifactTypeToPreview(type, getFileExtension(normalized)),
    confidence: 95,
    reason: "artifact",
  };
}
