...
async function main() {
  ...
  try {
    await waitFor(client, "Boolean(window.__legalworkControl)", 15000);
...
async function runPreflight(client) {
  ...
  const controlReady = await evaluate(client, "Boolean(window.__legalworkControl)");
  const actions = controlReady
    ? await evaluate(client, "window.__legalworkControl.listActions().map((action) => action.id)")
    : [];
  ...
  if (!controlReady) throw new Error("LegalWork control API is not available.");
...
async function ensureVoicePanel(client) {
  await evaluate(client, "window.__legalworkControl.setEnabled(true)");
  await evaluate(client, "window.localStorage.setItem('legalwork.extension.enabled.legalwork-voice', '1'); window.dispatchEvent(new CustomEvent('legalwork:extension-state-changed', { detail: { id: 'legalwork-voice', enabled: true } }))");
  let actions = await evaluate(client, "window.__legalworkControl.listActions().map((action) => action.id)");
  ...
  if (actions.includes("voice.panel.open")) {
    ...
    await waitFor(client, "window.__legalworkControl.listActions().some((action) => action.id === 'voice.inject_audio')", 8000);
    return;
  }
...
async function readProof(client) {
  return evaluate(client, `({
    href: location.href,
    narration: window.__legalworkControl?.snapshot?.().narration ?? "",
    route: window.__legalworkControl?.snapshot?.().route ?? "",
...
async function synthesizePcm16Base64(input) {
  const ffmpeg = await requireCommand("ffmpeg", "ffmpeg is required for generated voice audio. On Linux install it with: apt-get update && apt-get install -y ffmpeg espeak-ng");
  const tts = await findTtsCommand();
  const dir = await mkdtemp(join(tmpdir(), "legalwork-voice-cdp-"));
...
async function findTtsCommand() {
  ...
  throw new Error("No TTS command found. On Linux install one with: apt-get update && apt-get install -y espeak-ng ffmpeg");
}
...
async function pickTarget(baseUrl) {
  ...
  const target = pages.find((page) => (page.title ?? "").startsWith("LegalWork")) ??
