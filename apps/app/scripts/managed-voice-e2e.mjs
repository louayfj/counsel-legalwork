  ...
  const server = createServer((req, res) => {
    ...
    req.on("end", () => {
      ...
      res.end(JSON.stringify({
        ...
        tools: ["legalwork_snapshot", "legalwork_list_actions", "legalwork_execute_action"],
...
async function startLegalWorkServer({ directory, port, env }) {
  ...
  const child = spawn("bun", [
    ...
    env: { ...process.env, ...env, LEGALWORK_DEV_MODE: "1" },
  });
  let stdout = "";
  let stderr = "";
...
async function waitForServerHealthy(baseUrl) {
  ...
  throw new Error(`Timed out waiting for LegalWork server health: ${lastError}`);
}

await rm(outDir, { recursive: true, force: true });
await mkdir(outDir, { recursive: true });
const envDir = await mkdtemp(join(tmpdir(), "legalwork-managed-voice-e2e-"));
const mockBroker = await startMockBroker();
const port = await findFreePort();
const server = await startLegalWorkServer({
  directory,
  port,
  env: {
    LEGALWORK_ENV_STORE: join(envDir, "env.json"),
    LEGALWORK_TOKEN_STORE: join(envDir, "tokens.json"),
    LEGALWORK_API_KEY: "ow_inf_e2e",
    LEGALWORK_INFERENCE_BASE_URL: mockBroker.baseUrl,
  },
});
...
try {
  ...
  const owner = await step("owner token", async () => {
    const response = await fetch(`${server.baseUrl}/tokens`, {
      method: "POST",
      headers: { "x-legalwork-host-token": server.hostToken, "content-type": "application/json" },
  ...
  const session = await step("managed voice session", async () => {
    ...
    assert.equal(body.source, "legalwork-models");
    return body;
  });
