...
function hasBuiltInBrowserPrompt(haystack) {
  return (
    haystack.includes("built-in legalwork browser") ||
    haystack.includes("legalwork browser") ||
...
try {
  tmpdir = await mkdtemp(path.join(os.tmpdir(), "legalwork-browser-entry-"));
      ...
      if (req.method === "POST" && url.pathname.endsWith("/chat/completions")) {
        ...
        if (triesBuiltInBrowser) {
          ...
          writeSse(
            res,
            createTextStream(
              "Using the built-in LegalWork Browser to open example.com and read the page title.",
            ),
          );
        } else {
  ...
  await step("session.create", async () => {
    const session = await client.session.create({ title: "LegalWork browser-entry test" });
  ...
  await step("assert.built-in-browser-quickstart", async () => {
    assert.equal(sawBuiltInBrowserPrompt, true, "Expected browser quickstart prompt to use the built-in LegalWork Browser");
