...
function workspace(overrides: Partial<WorkspaceInfo> = {}): WorkspaceInfo {
  return {
    ...
    legalworkHostUrl: "https://worker.example.com/w/ws_remote",
    legalworkToken: "ow-token",
    ...overrides,
  };
}

function client(overrides: Partial<LegalworkServerClient> = {}): LegalworkServerClient {
  return {
...
describe("resolveRemoteWorkspaceConnectionTarget", () => {
  test("builds a host-scoped LegalWork target from saved worker credentials", () => {
    const target = resolveRemoteWorkspaceConnectionTarget(
      workspace({
        legalworkHostUrl: "https://worker.example.com",
        legalworkWorkspaceId: "ws_remote",
      }),
    );
  ...
  test("does not run LegalWork probes against non-LegalWork remote workspaces", () => {
    const target = resolveRemoteWorkspaceConnectionTarget(
      workspace({
        remoteType: "opencode",
        legalworkHostUrl: "",
    ...
    if (target.ok) return;
    expect(target.state.status).toBe("error");
    expect(target.state.message).toContain("LegalWork remote workers");
  });

  test("does not run LegalWork probes against stale LegalWork fields on non-LegalWork remotes", () => {
    const target = resolveRemoteWorkspaceConnectionTarget(
      workspace({
        remoteType: "opencode",
        legalworkHostUrl: "https://worker.example.com/w/ws_remote",
        legalworkToken: "owt_secret",
        baseUrl: "https://opencode.example.com",
      }),
    );
...
describe("testRemoteWorkspaceConnection", () => {
  ...
  test("reports a missing token after proving the worker endpoint is reachable", async () => {
    const result = await testRemoteWorkspaceConnection(workspace({ legalworkToken: "" }), {
      createClient: () => client(),
    });
    ...
    expect(result.state.message).toContain("Upgrade the LegalWork host");
    expect(result.state.message).toContain("chris@eigenweltlabs.com");
  });
  ...
  test("uses fallback LegalWork tokens saved on older workspace records", async () => {
    const result = await testRemoteWorkspaceConnection(
      workspace({
        legalworkToken: "",
        legalworkClientToken: "legacy-client-token",
      }),
      {
        createClient: (target) => {
...
describe("remote diagnostic identity", () => {
  ...
  test("changes when connection credentials change", () => {
    const before = getRemoteWorkspaceConnectionKey(workspace({ legalworkToken: "old-token" }));
    const after = getRemoteWorkspaceConnectionKey(workspace({ legalworkToken: "new-token" }));
