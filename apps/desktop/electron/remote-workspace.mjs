...
export function selectLegalworkWorkspaceForConnection(list, directory) {
...
export function legalworkWorkspaceDisplayName(workspace) {
  return (
    trim(workspace?.displayName) ||
    trim(workspace?.legalworkWorkspaceName) ||
    trim(workspace?.name) ||
    trim(workspace?.id) ||
    null
  );
