...
function defaultLegalworkConfig(targetDir, preset = "starter") {
  return {
    version: 1,
    workspace: {
...
export async function importWorkspaceConfig({ archivePath, targetDir, name }) {
  ...
  const legalworkPath = path.join(opencodeDir, "legalwork.json");
  let preset = "starter";
  ...
  if (await pathExists(legalworkPath)) {
    ...
    try {
      ...
      const config = defaultLegalworkConfig(targetDir, preset);
      await writeFile(legalworkPath, `${JSON.stringify(config, null, 2)}\n`, "utf8");
    }
  } else {
    const config = defaultLegalworkConfig(targetDir, preset);
