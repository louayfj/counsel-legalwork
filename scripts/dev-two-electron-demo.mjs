...
const demoRoot =
  process.env.LEGALWORK_ELECTRON_DEMO_ROOT?.trim() ||
  path.join(os.homedir(), ".legalwork", "two-electron-demo");
const appProfiles = {
  admin: {
    appIdentifier: "com.eigenweltlabs.legalwork.demo.admin",
    appName: "LegalWork Demo A",
  ...
  consumer: {
    appIdentifier: "com.eigenweltlabs.legalwork.demo.consumer",
    appName: "LegalWork Demo B",
...
function demoEnv(profile, bootstrapPath, port, cdpPort) {
  ...
  return {
    LEGALWORK_DATA_DIR: path.join(userDataDir, "legalwork-orchestrator-data"),
    LEGALWORK_DESKTOP_BOOTSTRAP_PATH: bootstrapPath,
    LEGALWORK_DESKTOP_DISABLE_WORKSPACE_RECOVERY: "1",
    VITE_DISABLE_LEGALWORK_MODELS: "1",
    LEGALWORK_ELECTRON_APP_IDENTIFIER: profile.appIdentifier,
    LEGALWORK_ELECTRON_APP_NAME: profile.appName,
    LEGALWORK_ELECTRON_REMOTE_DEBUG_PORT: cdpPort,
    LEGALWORK_ELECTRON_SKIP_SHARED_PREPARE: "1",
    LEGALWORK_ELECTRON_USERDATA: userDataDir,
    PORT: port,
  };
}
...
async function main() {
  ...
  const denStartup =
    adminPort === appProfiles.admin.port && consumerPort === appProfiles.consumer.port
      ? "pnpm demo:den"
      : `LEGALWORK_EXTRA_APP_PORTS=${adminPort},${consumerPort} pnpm dev:den`;
