#!/usr/bin/env bash
#
# legalwork-debug.sh — one-stop observability + lifecycle control for the
# LegalWork dev stack.
#
# Subcommands:
...
#   kill-orphans    remove orphan legalwork/opencode processes (ppid == launchd)
...
#   wait-healthy    block until legalwork-server reports /health = 200
#   reset           stop + wipe Vite dep cache + truncate log sink + start
#   restart         alias for reset
#
# Variant (LEGALWORK_DEV_VARIANT):
#   electron   (default) pnpm --filter @legalwork/desktop dev:electron
...
#   3. Tauri webview       (target/debug/LegalWork-Dev)  <-- never /Applications/
#   4. Electron main+helpers (node_modules/electron/...Electron.app)
#   5. Vite                (node node_modules/.../vite)
#   6. orchestrator + legalwork-server + opencode + opencode-router
...
#   - ~/Library/Application Support/com.eigenweltlabs.legalwork.dev/** (tokens,
#     workspaces registry, prefs). Use `reset-webview` for WebKit state.
#   - /Applications/LegalWork.app (prod build never targeted).
#
set -euo pipefail
...
DEV_LOG_FILE="${LEGALWORK_DEV_LOG_FILE:-$HOME/.legalwork/debug/legalwork-dev.log}"
PNPM_DEV_LOG="${LEGALWORK_PNPM_DEV_LOG:-/tmp/legalwork-test/pnpm-dev.log}"
PNPM_DEV_PID_FILE="${LEGALWORK_PNPM_DEV_PID:-/tmp/legalwork-test/pnpm-dev.pid}"
WAIT_HEALTHY_SECS="${LEGALWORK_WAIT_HEALTHY_SECS:-90}"
ELECTRON_CDP_PORT="${LEGALWORK_ELECTRON_REMOTE_DEBUG_PORT:-9823}"
...
DEV_VARIANT="${LEGALWORK_DEV_VARIANT:-electron}"
case "$DEV_VARIANT" in electron|tauri) ;; *)
  printf '[legalwork-debug] unknown LEGALWORK_DEV_VARIANT=%s (expected electron|tauri)\n' "$DEV_VARIANT" >&2
  exit 2
  ;;
esac
...
kill_by_pattern() {
  ...
  # webview (matched by its target/debug path, so prod LegalWork.app is safe).
  local pattern="$1"
  local pids
...
discover_legalwork_server_port() {
  ps -Ao command \
    | grep -E "(target/debug|apps/desktop/src-tauri/sidecars)/legalwork-server" \
    | grep -v grep \
    | grep -oE '\-\-port [0-9]+' \
    | head -1 \
    | awk '{print $2}' \
...
electron_renderer_stats() {
  ps -axo pid,ppid,pcpu,pmem,rss,command \
    | awk '/Electron Helper \(Renderer\)/ && /com\.eigenweltlabs\.legalwork/ && !/awk/ && !/grep/ {print; found=1} END {exit found ? 0 : 1}' \
    || true
}
...
probe_electron_page_cdp() {
  node <<'NODE'
const port = process.env.LEGALWORK_ELECTRON_REMOTE_DEBUG_PORT || "9823";
...
snapshot() {
  ...
  oc_port=$(ps -Ao command \
    | grep -E "(target/debug|apps/desktop/src-tauri/sidecars)/legalwork-orchestrator" \
    | grep -v grep \
  ...
  ps -Ao pid,ppid,command | awk '$2 == 1 && $3 ~ /legalwork-server|legalwork-orchestrator|opencode( |\/)|opencode-router/' | head
  ...
  if [[ -f "$DEV_LOG_FILE" ]]; then
    ...
    echo "  (no sink file yet — run the dev app with LEGALWORK_DEV_LOG_FILE set)"
  fi
}
...
kill_orphans() {
  local pids
  pids=$(ps -Ao pid,ppid,command | awk '$2 == 1 && $3 ~ /legalwork-server|legalwork-orchestrator|opencode( |\/)|opencode-router/ {print $1}')
  if [[ -z "$pids" ]]; then
    log "no orphans"
    return 0
  fi
...
diagnose_hang() {
  ...
  if [[ "$browser_json" == *"webSocketDebuggerUrl"* && "$target_json" == *"webSocketDebuggerUrl"* ]]; then
    if page_probe=$(LEGALWORK_ELECTRON_REMOTE_DEBUG_PORT="$ELECTRON_CDP_PORT" probe_electron_page_cdp 2>&1); then
  ...
  port=$(discover_legalwork_server_port)
  if [[ -n "$port" ]]; then
    echo "  legalwork-server port=$port"
    curl -sS --max-time 2 "http://127.0.0.1:$port/health" || echo "unreachable"
    echo
  else
    echo "  no legalwork-server port discovered"
  fi
  ...
  if [[ -n "$renderer_pid" && ( "$page_probe" == "failed" || "$high_cpu" == "true" ) ]]; then
    local sample_file="/tmp/legalwork-renderer-${renderer_pid}-$(date +%Y%m%dT%H%M%S).sample.txt"
    ...
    echo "  skipped (renderer responsive and CPU not high). Set LEGALWORK_FORCE_SAMPLE=1 not currently supported."
  else
...
stop() {
  ...
  kill_by_pattern "target/debug/LegalWork-Dev"
  ...
  # 5. legalwork-server / orchestrator / opencode / opencode-router for the
  ...
  kill_by_pattern "target/debug/legalwork-server"
  kill_by_pattern "target/debug/legalwork-orchestrator"
  kill_by_pattern "target/debug/opencode"
  kill_by_pattern "target/debug/opencode-router"
  kill_by_pattern "apps/desktop/src-tauri/sidecars/legalwork-server"
  kill_by_pattern "apps/desktop/src-tauri/sidecars/legalwork-orchestrator"
...
start() {
  ...
  case "$DEV_VARIANT" in
    electron)
      ...
      env LEGALWORK_DEV_LOG_FILE="$DEV_LOG_FILE" \
        nohup pnpm --filter @legalwork/desktop dev:electron >"$PNPM_DEV_LOG" 2>&1 &
      pid=$!
      ;;
    tauri)
...
wait_healthy() {
  ...
  while (( SECONDS < deadline )); do
    ...
    if [[ -n "$port" ]]; then
      ...
      if [[ "$code" == "200" ]]; then
        log "legalwork-server healthy on :$port"
        return 0
      fi
    fi
    sleep 1
  done
  log "legalwork-server did not become healthy within ${WAIT_HEALTHY_SECS}s" >&2
  return 1
}
...
reset_webview_state() {
  ...
  # the legalwork-workspaces.json registry or server-side tokens.
  local webkit_dir="$HOME/Library/WebKit/com.eigenweltlabs.legalwork.dev"
